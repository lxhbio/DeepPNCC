...
def plot_evaluating_metrics(metrics_list, path, fig_xlabel, fig_ylabel, fig_legend, filename, figsize=(13,9), color='orange'):
    figure, ax = plt.subplots(figsize=figsize, dpi=100)
    ...
    # plt.savefig('./output/'+'{}.png'.format(filename))   #修改之前 plt.savefig('{}.tif'.format(filename))
    plt.savefig(path+'/'+'{}.png'.format(filename))   #修改之前 plt.savefig('{}.tif'.format(filename))
    plt.close()
